...
// ======== Ícones por tipo no painel de simulação ========
...
export default function FluxoEditorPage() {
  ...
  async function salvar() {
    if (!fluxo) return
    setSalvando(true)
    // Sincroniza trigger da coluna com o nó de início
    const startNode = nodes.find(n => n.type === 'inicio')
    ...
    await supabase.from('fluxos').update({
      nodes: nodes.map(n => ({ id: n.id, type: n.type, position: n.position, data: Object.fromEntries(Object.entries(n.data).filter(([k]) => k !== 'nodeType')) })),
  ...
  // Executa simulação passo a passo com delay visual
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-zinc-950">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-800 bg-zinc-900 flex-shrink-0">
          <div className="flex items-center gap-3">
            <button onClick={() => router.push('/dashboard/fluxos')} className="text-zinc-400 hover:text-white transition-colors">
              <ArrowLeft size={18} />
            </button>
            <div>
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => { setSimAberta(v => !v); setSimSteps([]); setSimNodeAtivo(null) }}
      ...
      {/* ---- Painel de simulação ---- */}
      {simAberta && (
        <div className="w-80 flex-shrink-0 border-l border-zinc-800 bg-zinc-900 flex flex-col overflow-hidden">
          {/* Header */}
          ...
          <div className="p-3 border-t border-zinc-800">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="bg-violet-500 hover:bg-violet-400 disabled:opacity-40 text-white p-2 rounded-lg transition-colors"
              >
